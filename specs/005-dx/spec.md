...
開発者は claudecode-model の利用中にエラーが発生した場合、例外メッセージから問題の原因と解決方法を即座に把握する。全ての例外は `ClaudeCodeError` 派生の構造化例外として伝播し、エラー種別（`error_type`）・復旧可能性（`recoverable`）・解決ヒント（メッセージ本文）を含む。
...
- **FR-008**: システムは全パブリック関数・メソッドに型注釈を付与し、`Any` 型を使用しないこと
