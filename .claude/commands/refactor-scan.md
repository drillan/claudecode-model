...
similarity-pyを使用してコードベースの重複を検出し、優先度付けされたリファクタリング提案を含むMarkdownレポートを生成します。DRY原則に基づいて分析し、チームで共有可能な形式で保存します。

## Operating Constraints

**READ-ONLY SCAN + REPORT WRITE**: similarity-pyでスキャンしてレポートを生成しますが、ソースコード自体は変更しません。

**CLAUDE.md準拠**: 「TDD必須」「品質チェック」「禁止事項（コード重複・マジックナンバー）」のルールを厳守します。
...
**スキャンオプション**:
- `--path <path>`: スキャン対象パス（デフォルト: `src/claudecode_model`）
...
# 例3: /refactor-scan custom.md --path src/claudecode_model --threshold 0.90
# → 出力: custom.md, スキャン: src/claudecode_model, 閾値: 0.90

# 例4: /refactor-scan --path tests
# → 出力: ai_working/duplication-analysis-2025-01-15.md, スキャン: tests
```
...
```bash
# ソースコード全体スキャン
similarity-py src/claudecode_model --threshold 0.85 --min-lines 15 --cross-file --print

# テスト重複スキャン（オプション）
similarity-py tests --threshold 0.80 --min-lines 10 --cross-file --print
```
...
**CLAUDE.mdルール準拠**:
- **コード重複（3回以上は抽出）**: ❌ VIOLATION - [理由]
- **禁止事項**: [✅ 既存コード修正可能 / ⚠️ 要検討]
- **TDD必須**: [✅ テスト存在 / ⚠️ テスト不足]
- **禁止事項（マジックナンバー）**: [✅ 問題なし / ⚠️ ハードコーディング検出]
- **品質チェック**: [リファクタリング後の品質影響評価]
...
| ルール | Status | Issues Found | Recommendation |
|---------|--------|--------------|----------------|
| コード重複（3回以上は抽出） | [✅ / ⚠️ / ❌] | [件数] duplications ([行数] lines) | [推奨アクション] |
| 禁止事項 | [✅ / ⚠️] | [評価] | [推奨アクション] |
| TDD必須 | [✅ / ⚠️] | [評価] | [推奨アクション] |
| 禁止事項（マジックナンバー） | [✅ / ⚠️] | [件数] hardcoded values | [推奨アクション] |
| 品質チェック | [✅ / ⚠️] | [評価] | [推奨アクション] |
  ...
  duplication-check:
    ...
    steps:
      ...
      - name: Check duplications
        run: similarity-py src/claudecode_model --threshold 0.85 --min-lines 15
```
...
<details>
<summary>ソースコードスキャン結果（クリックで展開）</summary>
...
**CLAUDE.md Version**: [現在のCLAUDE.mdバージョン]
```
...
- **CLAUDE.md-first**: ルール違反は自動的にCRITICAL判定

### Quality Standards

- **品質チェック準拠**: 生成されたレポートもMarkdownとして高品質
- **禁止事項準拠**: ハードコーディングされた閾値は引数でオーバーライド可能
- **型安全準拠**: レポート内のコード例は型注釈を含む
